...
import { createResolver } from 'vite/dist/node/resolver.js';
...
// handle includes
function fileInclude(includes, filePath) {
...
// Bare exporter
function compareString(modify, data) {
    return modify ? '\n' + data.map((v) => `${v[0]}._path = ${v[1]}`).join('\n') : '';
}

function varTemplate(data, name) {
...
const globTransform = function (config) {
